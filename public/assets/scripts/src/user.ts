...
const buttonLogout = document.querySelector(
  ".button-logout"
) as HTMLButtonElement; // seleciona o botão de logout
...
if (profile) {
  ...
  const imgPreview = profile.querySelector(
    "#photo-preview"
  ) as HTMLImageElement; // prévia da foto selecionada
  const inputFile = profile.querySelector("#file") as HTMLInputElement; // input da foto
  const buttonChoose = profile.querySelector(
    ".choose-photo"
  ) as HTMLButtonElement; // botão de alterar a foto
  ...
  const inputDistrict = form.querySelector(
    "[name=district]"
  ) as HTMLInputElement;
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (auth.currentUser) {
      const { name, email, phone, adress, number, district, city } =
        getFormValues(form);
      updateProfile(auth.currentUser, {
        displayName: name,
      });
